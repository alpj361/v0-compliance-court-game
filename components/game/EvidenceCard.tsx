...
// ── BankRecordRenderer — inlined to avoid Turbopack module-resolution issues ──
function BankRecordRenderer({ meta, onRead }: { meta: BankRecordMeta; onRead?: () => void }) {
  ...
  return (
    ...
    >
      <button className="w-full flex items-start gap-3 px-4 py-3 text-left" onClick={() => setOpened(!opened)}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <span className={cn('font-serif font-semibold', opened ? 'text-green-400' : 'text-foreground')}>{meta.systemLabel}</span>
            {opened ? <ChevronUp size={14} className="text-green-700 shrink-0" /> : <ChevronDown size={14} className="text-muted-foreground shrink-0" />}
          </div>
      ...
      {opened && (
        <div className="border-t border-green-900/40">
          <div className="px-4 py-1.5 border-b border-green-900/30 flex items-center justify-between" style={{ background: '#0a1a0f' }}>
            ...
            <span className="text-[9px] text-green-700 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 inline-block animate-pulse" />CONNECTED
            </span>
          </div>
          ...
          <div className="px-4 py-3 space-y-1.5" style={{ background: '#060f0a' }}>
            {meta.rows.slice(0, visibleRows).map((row, i) => (
              <div key={i} className="flex items-center justify-between gap-4">
                ...
                <span className={cn('text-right font-mono', row.highlight ? 'text-court-red font-bold tracking-wide' : 'text-green-300')}>{row.value}</span>
              </div>
            ))}
            {visibleRows < meta.rows.length && <span className="inline-block w-2 h-3 bg-green-500/70 animate-pulse" />}
          </div>
          {meta.footer && visibleRows >= meta.rows.length && (
            <div className="px-4 py-2.5 border-t border-green-900/30 text-[10px] text-green-700/70 leading-relaxed" style={{ background: '#0a1a0f' }}>{meta.footer}</div>
          )}

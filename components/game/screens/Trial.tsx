...
export function Trial({ state, dispatch, currentDialogue, isChoicePoint, clearShake }: TrialProps) {
  ...
  // Stop timer when choice made
  const handleChoice = useCallback((choice: DialogueLine['choices'] extends (infer C)[] | undefined ? NonNullable<C> : never) => {
    if (timerRef.current) clearInterval(timerRef.current)
    dispatch({
      type: 'CHOOSE_ANSWER',
      payload: {
        ...
        {courtRecordOpen && (
          <div
            className="absolute top-12 left-1/2 -translate-x-1/2 z-20 w-full max-w-lg max-h-[55vh] overflow-y-auto
                       bg-court-navy-mid border border-court-gold/40 rounded-sm shadow-xl p-4 flex flex-col gap-3"
          >
